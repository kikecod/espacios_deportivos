...
export class UsuarioRolService {
  ...
  async create(createUsuarioRolDto: CreateUsuarioRolDto) {
    const usuario = await this.usuarioRepository.findOneBy({
      id_usuario: createUsuarioRolDto.id_usuario,
    });
    const rol = await this.rolRepository.findOneBy({
      id_rol: createUsuarioRolDto.id_rol,
    });
    if (!usuario) {
  ...
  async findOne(idU: number, idR: number) {
    const existsUsuario = await this.usuarioRolRepository.exist({
      where: { id_usuario: idU },
    });
    const existsRol = await this.usuarioRolRepository.exist({
      where: { id_rol: idR },
    });
    if (!existsUsuario) {
      throw new NotFoundException('Usuario no encontrado');
  ...
  async update(
    ...
    return await this.usuarioRolRepository.update(
      { id_usuario: idU, id_rol: idR },
      updateUsuarioRolDto,
    );
  }
  ...
  async restore(idU: number, idR: number) {
    ...
    return await this.usuarioRolRepository.restore({
      id_usuario: idU,
      id_rol: idR,
    });
  }
