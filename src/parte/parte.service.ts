...
export class ParteService {
  ...
  async create(createParteDto: CreateParteDto) {
    const cancha = await this.canchaRepository.findOneBy({
      id_cancha: createParteDto.id_cancha,
    });
    const disciplina = await this.disciplinaRepository.findOneBy({
      id_disciplina: createParteDto.id_disciplina,
    });
    if (!cancha) {
  ...
  async findOne(idC: number, idD: number) {
    const existsCancha = await this.parteRepository.exist({
      where: { id_cancha: idC },
    });
    const existsDisciplina = await this.parteRepository.exist({
  ...
  async update(idC: number, idD: number, updateParteDto: UpdateParteDto) {
    ...
    return await this.parteRepository.update(
      { id_cancha: idC, id_disciplina: idD },
      updateParteDto,
    );
  }
