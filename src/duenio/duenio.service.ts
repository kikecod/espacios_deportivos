...
export class DuenioService {
  ...
  async create(createDuenioDto: CreateDuenioDto): Promise<Duenio> {
    ...
    const persona = await this.personaRepository.findOneBy({
      id_persona: createDuenioDto.id_persona_d,
    });
    if (!persona) {
    ...
    if (!usuario) {
      throw new NotFoundException(
        'Usuario asociado a la persona no encontrado',
      );
    }
  ...
  async findOne(id: number) {
    const exsit = await this.duenioRepository.exists({
      where: { id_persona_d: id },
    });
    if (!exsit) {
  ...
  async restore(id: number) {
    const exists = await this.duenioRepository.exist({
      where: { id_persona_d: id },
      withDeleted: true,
    });
    if (!exists) {
