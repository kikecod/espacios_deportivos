...
const DEV_CLIENT_EMAIL =
  process.env.DEV_CLIENT_EMAIL || 'dev_cliente@local.test';
...
const DEV_DUENIO_EMAIL =
  process.env.DEV_DUENIO_EMAIL || 'dev_duenio@local.test';
const DEV_DUENIO_USER = process.env.DEV_DUENIO_USER || 'dev_duenio';
const DEV_DUENIO_PASS = process.env.DEV_DUENIO_PASS || 'duenio123';

const DEV_CONTROLADOR_EMAIL =
  process.env.DEV_CONTROLADOR_EMAIL || 'dev_controlador@local.test';
const DEV_CONTROLADOR_USER =
  process.env.DEV_CONTROLADOR_USER || 'dev_controlador';
const DEV_CONTROLADOR_PASS =
  process.env.DEV_CONTROLADOR_PASS || 'controlador123';
...
async function findExistingColumn(
  ...
  for (const c of candidates) {
    const found = existing.find(
      (e: string) => e.toLowerCase() === c.toLowerCase(),
    );
...
async function chooseUserRoleTable(): Promise<{
  table: string;
  colUser: string;
  colRol: string;
}> {
  ...
  for (const t of candidates) {
    if (await tableExists(t)) {
      const colUser =
        (await findExistingColumn(t, ['id_usuario'])) ?? 'id_usuario';
      const colRol = (await findExistingColumn(t, ['id_rol'])) ?? 'id_rol';
      return { table: t, colUser, colRol };
    }
  }
  throw new Error(
    'No existe tabla de vinculación usuario-rol (usuarios_roles / usuario_rol)',
  );
}
...
async function ensureRoles() {
  ...
  for (const r of values) {
    const found = await client.query(
      `SELECT ${ident(id_rolCol)} FROM roles WHERE ${ident(rolCol)} = $1`,
      [r],
    );
    if (found.rowCount === 0) {
...
async function getUserByEmail(
  email: string,
): Promise<{ id_usuario: number; id_persona: number | null } | null> {
  const id_usuarioCol =
    (await findExistingColumn('usuarios', ['id_usuario'])) ?? 'id_usuario';
  const correoCol =
    (await findExistingColumn('usuarios', ['correo'])) ?? 'correo';
  const id_personaCol =
    (await findExistingColumn('usuarios', ['id_persona'])) ?? 'id_persona';
  const rs = await client.query(
  ...
  return {
    id_usuario: Number(rs.rows[0].id_usuario),
    id_persona:
      rs.rows[0].id_persona === null ? null : Number(rs.rows[0].id_persona),
  };
}
...
async function createUserAdmin(maybePersonaId: number | null): Promise<number> {
  ...
  const id_personaCol =
    (await findExistingColumn('usuarios', ['id_persona'])) ?? 'id_persona';
  const usuarioCol =
    (await findExistingColumn('usuarios', ['usuario'])) ?? 'usuario';
  const correoCol =
    (await findExistingColumn('usuarios', ['correo'])) ?? 'correo';
  const correoVerifCol =
    (await findExistingColumn('usuarios', ['correo_verificado'])) ??
    'correo_verificado';
  const hashCol =
    (await findExistingColumn('usuarios', ['hash_contrasena'])) ??
    'hash_contrasena';
  const estadoCol =
    (await findExistingColumn('usuarios', ['estado'])) ?? 'estado';
...
async function createUserGeneric(
  username: string,
  email: string,
  password: string,
  maybePersonaId: number | null = null,
): Promise<number> {
  const id_usuarioCol =
    (await findExistingColumn('usuarios', ['id_usuario'])) ?? 'id_usuario';
  const id_personaCol =
...
async function ensureUserHasRole(
  id_usuario: number,
  roleName: 'ADMIN' | 'CLIENTE' | 'DUENIO' | 'CONTROLADOR',
) {
...
async function createPersonaMinimal(): Promise<number> {
  if (!(await tableExists('personas')))
    throw new Error('Tabla personas no existe');

  const id_personaCol =
    (await findExistingColumn('personas', ['id_persona'])) ?? 'id_persona';
  const nombresCol =
    (await findExistingColumn('personas', ['nombres'])) ?? 'nombres';
  const paternoCol =
    (await findExistingColumn('personas', ['paterno'])) ?? 'paterno';
  const maternoCol =
    (await findExistingColumn('personas', ['materno'])) ?? 'materno';
  const telefonoCol =
    (await findExistingColumn('personas', ['telefono'])) ?? 'telefono';
  const fechaCol =
    (await findExistingColumn('personas', ['fecha_nacimiento'])) ??
    'fecha_nacimiento';
  const generoCol =
    (await findExistingColumn('personas', ['genero'])) ?? 'genero';
...
async function ensureCliente(personaId: number) {
  ...
  const r = await client.query(`SELECT 1 FROM cliente WHERE id_cliente = $1`, [
    personaId,
  ]);
  if ((r.rowCount ?? 0) > 0) {
...
async function ensureDuenio(personaId: number) {
  ...
  const r = await client.query(`SELECT 1 FROM duenio WHERE id_persona_d = $1`, [
    personaId,
  ]);
  if ((r.rowCount ?? 0) > 0) {
...
async function ensureControlador(personaId: number) {
  if (!(await tableExists('controlador'))) {
    console.warn(
      'Tabla controlador no existe, omitiendo creación de controlador',
    );
    return;
  }
  const r = await client.query(
    `SELECT 1 FROM controlador WHERE id_persona_ope = $1`,
    [personaId],
  );
  if ((r.rowCount ?? 0) > 0) {
...
async function run() {
  ...
  try {
    ...
    let id_dev: number;
    if (!devUser) {
      id_dev = await createUserGeneric(
        DEV_CLIENT_USER,
        DEV_CLIENT_EMAIL,
        DEV_CLIENT_PASS,
        null,
    ...
    if (!devUser) {
      id_duenio_user = await createUserGeneric(
        DEV_DUENIO_USER,
        DEV_DUENIO_EMAIL,
        DEV_DUENIO_PASS,
        null,
    ...
    if (!devUser) {
      id_controlador_user = await createUserGeneric(
        DEV_CONTROLADOR_USER,
        DEV_CONTROLADOR_EMAIL,
        DEV_CONTROLADOR_PASS,
        null,
      );
    } else {
      id_controlador_user = devUser.id_usuario;
      console.log(
        'Dev controlador already exists with id',
        id_controlador_user,
      );
    }
    ...
    if (!devUser) {
      id_all_user = await createUserGeneric(
        DEV_ALL_USER,
        DEV_ALL_EMAIL,
        DEV_ALL_PASS,
        null,
